...
function ChatPage() {
  ...
  const fetchContacts = async () => {
    try {
      ...
      if (result.success) {
        ...
        const filteredContacts = result.response.users.filter(
  ...
  const fetchMessages = async (contactId) => {
    ...
    try {
      ...
      if (conversationsMap[contactId] && conversationsMap[contactId].length > 0) {
  ...
  const currentMessages = selectedContact ? (conversationsMap[selectedContact.id] || []) : [];
      ...
      {selectedContact ? (
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50">
          <div className="text-center">
            <h2 className="text-xl font-semibold text-gray-700">Select a contact to start chatting</h2>
          </div>
        </div>
      )}
    </div>
